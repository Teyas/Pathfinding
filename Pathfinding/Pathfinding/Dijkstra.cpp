...
namespace djk
{
	bool FindPath(const Model& model, std::vector<int>& out_path)
	{
		...
		const int map_size = (int)map.data.size();
		...
		auto queue_compare_func = [](const QueueInfo& lhs, const QueueInfo& rhs) { return lhs.cost > rhs.cost; };
		...
		int current_node_idx = invalid_index;
		while (node_queue.empty() == false)
		{
			...
			{
				// backtrack path
				unsigned parent_idx = node_info_list[current_node_idx].parent_index;
